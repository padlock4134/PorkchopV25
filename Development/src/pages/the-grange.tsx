...
const TheGrange: NextPage = () => {
  ...
  const localProducts: LocalProduct[] = [
    ...
    },
    {
      ...
      description: 'Tender, organic baby spinach leaves. Perfect for salads, sautés, or smoothies.',
      price: '$3.99/bunch',
      category: 'produce',
      subcategory: 'greens',
      producer: 'Green Acres Farm',
    ...
    },
    {
      ...
      description: 'Unfiltered, raw wildflower honey with complex floral notes.',
      price: '$8.99/jar',
      category: 'specialty',
      subcategory: 'honey',
      producer: 'Wild Harvest',
  ...
  const localProducers: LocalProducer[] = [
    ...
    },
    {
      ...
      description: 'Family farm practicing regenerative agriculture. Known for exceptional heirloom vegetables and seasonal fruits.',
      image: 'green-acres',
      rating: 4.6,
      distance: '3.5 miles'
    },
    ...
    },
    {
      ...
      type: 'specialty',
      location: 'Eastside',
      specialty: 'Foraged goods',
      description: 'Specializing in wild-harvested and foraged foods including mushrooms, berries, and honey.',
      image: 'wild-harvest',
      rating: 4.5,
      distance: '4.2 miles'
    }
  ...
  // Generate a placeholder image for producers
  const getProducerImage = (producer: LocalProducer) => {
    const getTypeColor = (type: LocalProducer['type']): string => {
      switch(type) {
        case 'butcher': return 'bg-gradient-to-br from-red-700 to-red-900';
        case 'farmer': return 'bg-gradient-to-br from-green-600 to-green-800';
        case 'cheesemaker': return 'bg-gradient-to-br from-yellow-400 to-yellow-600';
        case 'baker': return 'bg-gradient-to-br from-orange-400 to-orange-600';
        case 'specialty': return 'bg-gradient-to-br from-purple-600 to-purple-800';
        default: return 'bg-gradient-to-br from-butcher-600 to-butcher-800';
      }
    };

    const getTypeIcon = (type: LocalProducer['type']): string => {
      switch(type) {
        case 'butcher': return '🔪';
        case 'farmer': return '🌱';
        case 'cheesemaker': return '🧀';
        case 'baker': return '🍞';
        case 'specialty': return '🌿';
        default: return '🏡';
      }
    ...
    return (
      <div className={`w-16 h-16 rounded-full flex items-center justify-center ${getTypeColor(producer.type)}`}>
        <span className="text-2xl">{getTypeIcon(producer.type)}</span>
      </div>
    );
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {localProducts.filter(p => p.featured).map(product => (
            <div key={product.id} className="bg-white rounded-lg shadow-vintage overflow-hidden border border-vintage-100 hover:shadow-vintage-lg transition-all duration-200">
              <div className="h-48">
                {getProductImage(product)}
              </div>
              <div className="p-6">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-xl font-semibold text-butcher-800">{product.name}</h3>
                  <span className="text-porkchop-600 font-bold">{product.price}</span>
                </div>
                <p className="text-butcher-600 text-sm mb-4">{product.description}</p>
                <div className="flex flex-wrap gap-2 mb-4">
                  {product.tags.map((tag, idx) => (
                    <span key={idx} className="text-xs bg-vintage-100 text-butcher-700 px-2 py-1 rounded-full shadow-tag">
                      {tag}
                    </span>
                  ))}
                </div>
          ...
          {filteredProducts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredProducts.map(product => (
                <div key={product.id} className="bg-white rounded-lg shadow-vintage hover:shadow-vintage-lg transition-all duration-200 border border-vintage-100 overflow-hidden">
                  <div className="h-40">
                    {getProductImage(product)}
                  </div>
                  <div className="p-4">
                    ...
                    <div className="flex flex-wrap gap-1 mb-3">
                      {product.tags.slice(0, 3).map((tag, idx) => (
                        <span key={idx} className="text-xs bg-vintage-100 text-butcher-700 px-2 py-0.5 rounded-full shadow-tag">
                          {tag}
                        </span>
                      ))}
                    </div>
                    ...
                    <button 
                      className={`w-full px-3 py-1.5 rounded-lg text-sm font-medium transition-colors duration-200 ${
                        product.inStock 
                          ? 'bg-porkchop-500 text-white hover:bg-porkchop-600' 
                          : 'bg-gray-200 text-gray-500 cursor-not-allowed'
                      }`}
      ...
      <div className="mb-16">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-2xl font-semibold text-butcher-900 font-serif">Local Producers</h2>
          <button className="text-porkchop-600 hover:text-porkchop-700 font-medium">
            View All →
          </button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {localProducers.map(producer => (
            <div key={producer.id} className="bg-white rounded-lg shadow-vintage hover:shadow-vintage-lg transition-all duration-200 border border-vintage-100 overflow-hidden">
              <div className="p-6">
                <div className="flex items-center mb-4">
                  {getProducerImage(producer)}
                  <div className="ml-4">
                    <h3 className="text-lg font-semibold text-butcher-800">{producer.name}</h3>
                    <div className="flex items-center text-sm">
                      <span className="text-porkchop-500 mr-1">{producer.rating.toFixed(1)}</span>
                      <div className="flex">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <svg 
                            key={i} 
                            className={`w-4 h-4 ${i < Math.round(producer.rating) ? 'text-porkchop-500' : 'text-gray-300'}`} 
                            fill="currentColor" 
                            viewBox="0 0 20 20"
                          >
                ...
                <p className="text-sm text-butcher-600 mb-4">{producer.description}</p>
                <div className="flex items-center justify-between text-sm">
                  <span className="flex items-center text-butcher-500">
                    <svg className="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" />
                    </svg>
                    {producer.location} ({producer.distance})
                  </span>
                  <span className="text-xs bg-vintage-100 text-butcher-700 px-2 py-1 rounded-full shadow-tag">
                    {producer.specialty}
                  </span>
                </div>
              </div>
              <div className="px-6 py-4 bg-vintage-50 border-t border-vintage-100">
                <button className="w-full px-4 py-2 bg-porkchop-500 text-white rounded-lg font-medium hover:bg-porkchop-600 transition-colors duration-200">
                  View Products
                </button>
              </div>
            </div>
